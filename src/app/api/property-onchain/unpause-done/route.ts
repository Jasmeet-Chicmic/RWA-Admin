...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as UnpauseDoneRequestBody;
    ...
    const upstreamResponse = await fetch(
      `${baseUrl.replace(/\/$/, "")}/v1/property-onchain/unpause-done`,
      {
        method: "POST",
