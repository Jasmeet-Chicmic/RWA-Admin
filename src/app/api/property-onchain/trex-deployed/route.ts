...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as TrexDeployedRequestBody;
    ...
    const upstreamResponse = await fetch(
      `${baseUrl.replace(/\/$/, "")}/v1/property-onchain/trex-deployed`,
      {
        method: "POST",
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to report trex deployment",
        statusCode: 500,
      },
      { status: 500 },
    );
