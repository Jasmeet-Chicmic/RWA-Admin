...
export async function POST(req: Request) {
  try {
    const body = (await req.json()) as VaultDeployedRequestBody;
    ...
    const upstreamResponse = await fetch(
      `${baseUrl.replace(/\/$/, "")}/v1/property-onchain/vault-deployed`,
      {
        method: "POST",
    ...
    return NextResponse.json(
      {
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to report vault deployment",
        statusCode: 500,
      },
      { status: 500 },
    );
