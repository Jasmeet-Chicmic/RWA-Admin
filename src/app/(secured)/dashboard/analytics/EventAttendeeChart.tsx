...
import { fetchEventAttendeeAction, EventAttendeeData } from "@/api/dashboard";
...
const EventAttendeeChart = ({
  initialFromDate = "",
  initialToDate = "",
}: EventAttendeeChartProps) => {
  const t = useTranslations("dashboard");
  const { resolvedTheme } = useTheme();
  const [data, setData] = useState<EventAttendeeData[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetchEventAttendeeAction({
          filterType,
          groupBy,
          startDate: fromDate,
          endDate: toDate,
        ...
        console.error("Error fetching event attendee reports:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const chartOptions: ApexOptions = {
    ...
    tooltip: {
      y: {
        formatter: (value: number) => `${value} ${t("eventAttendeeTooltip")}`,
      },
    },
  };
  ...
  return (
    <AnalyticsChartCard
      title={t("eventAttendeeTitle")}
      subtitle={t("eventAttendeeSubtitle")}
      loading={loading}
...
export default EventAttendeeChart;
